...
export default function ReportTable({ data }: { data: any[] }) {
    ...
    const filtered = useMemo(() => {
        ...
        if (search.trim()) {
            ...
            rows = rows.filter((r) =>
                [r.first_name, r.last_name, r.remark]
                    .filter(Boolean)
    ...
    return (
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm max-h-[70vh] overflow-y-auto">
                        ...
                        <tr>
                            {columns.map((col) => (
                                <th
                                    ...
                                    className="cursor-pointer select-none whitespace-nowrap px-4 py-3 text-left text-sm font-semibold uppercase tracking-wide text-gray-500 hover:text-gray-700"
                                >
